...
Views separate your controller / application logic from your presentation logic and are stored in the `assets/views` directory. When using Tiny, view templates are written using a built-in templating language similar to blade. A simple view might look something like this:
...
You can create a file within `assets/views` with the `.html.php` extension to create a view. Views are rendered using Tiny's built-in templating language that provides several powerful features like easily echoing values, writing if conditions are more.
...
Tiny comes with a pre-built layout view with tailwind CSS. In order to use it, remove the `html` tag and add the following at the top:
...
### Using Widgets
Tiny comes with several built-in widgets to help faster prototyping. A full list of these widgets will be available soon.
